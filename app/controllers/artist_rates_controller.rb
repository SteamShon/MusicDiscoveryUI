class ArtistRatesController < ApplicationController
  def create
    @rate = ArtistRate.new(params[:artist_rate])
    params[:user_id] = @rate.user_id
    params[:tag_id] = @rate.tag_id
    @user_rated = current_user.artist_rates(@rate.tag_id)
  ...
  def destroy
    @rate = ArtistRate.find(params[:id])
    params[:user_id] = @rate.user_id
    params[:tag_id] = @rate.tag_id
    @rate.destroy
    @user_rated = current_user.artist_rates(@rate.tag_id)
